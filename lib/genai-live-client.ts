...
export class GenAILiveClient {
  private readonly events = new EventEmitter<LiveClientEventTypes>();
  ...
  public send(parts: Part | Part[], turnComplete: boolean = true) {
    if (this._status !== 'connected' || !this.session) {
      this.events.emit('error', new ErrorEvent('Client is not connected'));
      return;
    }
  ...
  protected onMessage(message: LiveServerMessage) {
    ...
    if (message.toolCall) {
      ...
      this.events.emit('toolcall', message.toolCall);
      return;
    }
    if (message.toolCallCancellation) {
      this.log('receive.toolCallCancellation', message);
      this.events.emit('toolcallcancellation', message.toolCallCancellation);
      return;
    }
    ...
    if (message.serverContent) {
      ...
      if (serverContent.modelTurn) {
        ...
        this.events.emit('content', content);
